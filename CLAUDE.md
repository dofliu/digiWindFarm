...
- cup-anemometer overspeeding bias — fixed: IEC 61400-12-1 Annex H statistical bias `bias = 1 + 1.5·TI_local²` (Kristensen 1998 / Pedersen 2006) applied to `nac_anem_raw` after the #117/#125 NTF block; TI_local quadrature combine of (`effective_ti × _local_ti_multiplier`) and `_wake_added_ti`; clamp ≤+10%; rotor_speed ≤ 1 RPM → bias=1.0; first time three TI paths (#99 ABL, #91 pocket, #103 wake-added) couple back into the sensor reading; closes IEC 61400-12-1 mean (Annex D, #117) + Glauert (#125) + statistical (Annex H, #127) chain; no new SCADA tag — see #127
...
- yaw-skew Glauert correction for NTF + WVTF — done (#125): under yaw misalignment γ Glauert combined-momentum / Coleman skewed-wake reduces the axial induction at the anemometer position to `a_eff = a · cos²(γ)`, and the swirl plane projects onto the nacelle plane as `θ_s_eff = θ_s · cos(γ)`; reuses `yaw_out["yaw_error"]` already in `step()`; γ clamped ±45°; γ=0 reproduces #117/#119 baseline exactly (cos²=cos=1). Self-test (14/14 PASS): Region 2 / γ=0° → 0.842 (matches #117), γ=15° → 0.852 (NTF correction shrinks 6.7% per cos² law), γ=30° → 0.881 (correction shrinks 25%), γ=45° → 0.921 (correction halved); WVTF / γ=0° → +3.36°, γ=15° → +3.24°, γ=30° → +2.91°, γ=45° → +2.38°; γ symmetry (cos²) and ±45° clamp verified; γ↑ → factor↑ and bias↓ monotonic. References: Glauert (1935) "Airplane Propellers" in *Aerodynamic Theory* (Durand) §VII; Burton, Sharpe, Jenkins, Bossanyi (2011) *Wind Energy Handbook* 2nd ed. §3.7, §3.10; Castillo-Negro et al. (2008) — Coleman skewed wake correction. Closes the IEC 61400-12-1/2 chain under non-zero yaw error; pairs with `yaw_misalignment` fault for the "vane miscalibration → systematic yaw error" diagnostic. No new SCADA tag — observable via `(WMET_WSpeedRaw / WMET_WSpeedNac − 1) × WYAW_YwVn1AlgnAvg5s` and `(WMET_WDirRaw − WMET_WDirAbs) × WYAW_YwVn1AlgnAvg5s` correlations. Also cleaned up the duplicate `WMET_WDirRaw` registration left by the #119 merge in both `turbine_physics.py::step()` output dict and `scada_registry.py` `_TAGS` list (104 physics tags / 110 total registry entries unchanged). — see #125
- cup-anemometer overspeeding bias — done (#127): IEC 61400-12-1 Annex H statistical bias `bias = 1 + 1.5·TI_local²` (Kristensen 1998 / Pedersen 2006) applied multiplicatively to `nac_anem_raw` after the #117/#125 NTF block. TI_local computed in quadrature: `√((effective_ti·_local_ti_multiplier)² + _wake_added_ti²)` — combines #99 ABL TI multiplier × #91 pocket multiplier with #103 wake-added TI as independent stochastic sources. k=1.5 for heated Risø Class 1 cup (industry typical); sonic anemometers k≈0. Clamp ≤+10% (physical upper bound). Rotor stopped/parked → bias=1.0 (cup not turning). 19/19 self-test PASS: TI=0 → 1.000 (#117 baseline preserved); TI=0.10 → +1.5%; TI=0.15 → +3.4%; TI=0.20 → +6.0%; TI≥0.27 → clamped at +10%. First time three TI paths (#99/#91/#103) couple back into the sensor reading. No new SCADA tag — observable via `WMET_WSpeedRaw / WMET_WSpeedNac × WMET_LocalTi × WMET_WakeTi`. Closes IEC 61400-12-1 mean (Annex D, #117) + Glauert γ (#125) + statistical (Annex H, #127) chain — see #127
