...
- Glauert yaw skewed-flow correction on NTF + WVTF — fixed: `a_skew = a · cos²(γ)` (Glauert 1935 / Burton et al. 2011 §3.10) for NTF and `θ_s_eff = (Ct/(2·λ)) · cos(γ)` for WVTF (planar-projection geometry). Closes the IEC 61400-12-1/2 chain under non-zero yaw (`yaw_misalignment` faults, ±5° dead-band, gusts). γ clamped ±45°. γ=0 baseline preserved exactly; γ=15° → NTF reduction ×0.933, WVTF bias ×0.966; γ=30° → 0.75 / 0.866; γ=45° → 0.50 / 0.707. Reuses `yaw_out["yaw_error"]` already in `step()` — no new state, no new SCADA tag (observable via `WMET_WSpeedRaw / WMET_WSpeedNac × WYAW_YwVn1AlgnAvg5s`). Also clears two duplicate `WMET_WDirRaw` keys (`turbine_physics.py:777`, `scada_registry.py:151`) that were leftover from the #119 merge — see #125
- cup-anemometer overspeeding bias — fixed: IEC 61400-12-1 Annex H (Kristensen 1998 Risø-R-1024, Pedersen 2006 Risø-R-1473) `bias = 1 + 1.5·TI_local²` with TI_local from quadrature combine of (`effective_ti` × `_local_ti_multiplier`) and `_wake_added_ti`; clamp ≤+10%; only applies when rotor turning. TI=0.10 → +1.5%; TI=0.15 → +3.4%; TI=0.20 → +6.0%; TI≥0.27 clamped at +10%. Closes IEC 61400-12-1 Annex D (mean) + Annex H (statistical) for the cup-style nacelle anemometer; first time three TI paths (`#99 ABL`, `#91 pocket`, `#103 wake-added`) couple back into the sensor reading. No new SCADA tag, observable via `WMET_WSpeedRaw / WMET_WSpeedNac × WMET_LocalTi × WMET_WakeTi` — see #127
...
- Glauert yaw skewed-flow correction on NTF + WVTF — done: NTF `V_raw = V_∞ · (1 − 0.55·a·cos²γ)` (Glauert 1935 / Burton et al. 2011 §3.10) and WVTF `θ_s_eff = θ_s·cos γ` (planar-projection geometry, Burton §3.7); γ from existing `yaw_out["yaw_error"]` (so `yaw_misalignment` faults, ±5° dead-band, gust transients all flow through automatically), clamped ±45° (validity of skewed-momentum theory); γ=0 → exact #117/#119 baseline preserved; γ=15° → NTF reduction × 0.933 (Region 2 V_raw 8.42 → 8.52 m/s @ V_∞=10), WVTF bias × 0.966 (3.36° → 3.24°); γ=30° → 0.75 / 0.866; γ=45° → 0.50 / 0.707; symmetry γ→−γ verified, monotonicity |γ|↑ → factor↑ / bias↓ verified; reuses `cos γ` once for both blocks (no extra cost, no new state, no new SCADA tag, no new RNG); also clears two duplicate `WMET_WDirRaw` keys leftover from the #119 merge (`turbine_physics.py:777` and `scada_registry.py:151`); 18/18 self-test PASS — see #125
- cup-anemometer overspeeding bias — done: IEC 61400-12-1 Annex H statistical bias `bias = 1 + 1.5·TI_local²` (Kristensen 1998 Risø-R-1024; Pedersen 2006 Risø-R-1473) applied to `nac_anem_raw` after the #117/#125 NTF block. TI_local computed in quadrature: `√((effective_ti·_local_ti_multiplier)² + _wake_added_ti²)` — combines #99 ABL TI multiplier × #91 pocket multiplier with #103 wake-added TI as independent stochastic sources. k=1.5 for heated Risø Class 1 cup (sonic anemometers k≈0). Clamp ≤+10% (physical upper bound). Operating-state branching: stopped/parked rotor → bias = 1.0 (cup not turning, no overspeeding); producing/starting with `rotor_speed > 1 RPM` applies the bias. TI=0 → 1.000 (#117 baseline preserved); TI=0.06 → +0.5%; TI=0.10 → +1.5%; TI=0.15 → +3.4%; TI=0.20 → +6.0%; TI=0.30 → clamped to +10%. Closes IEC 61400-12-1 mean (Annex D, #117) + statistical (Annex H, #127) chain for cup-style nacelle anemometers; first time three TI paths (`#99 ABL`, `#91 pocket`, `#103 wake-added`) couple back into the sensor reading. Reuses existing `effective_ti` from `engine.py:112` (no extra cost, no new state, no new RNG). No new SCADA tag — observable via `WMET_WSpeedRaw / WMET_WSpeedNac × WMET_LocalTi × WMET_WakeTi`. 19/19 self-test PASS. References: Kristensen, L. (1998) "Cup anemometer behavior in turbulent environments" *J. Atmos. Oceanic Technol.* 15, 5–17; Pedersen, T. F. et al. (2006) Risø-R-1473; IEC 61400-12-1 ed.2 §6.3.4 + Annex H — see #127
