...
- regression: #125 cleanup actually removed the *only* `WMET_WDirRaw` registry entry (no duplicate existed in `scada_registry.py`; the F601 was from the dict literal alone), silently dropping the IEC 61400-12-2 Annex E WVTF sensor reading from SCADA output. #127 merge separately left orphaned `yaw_skew_rad` / `yaw_skew_cos` / `induction_a_yawed` lines duplicating the `cos_gamma` block (ruff F841 × 2). 2026-05-03: restored the canonical `WMET_WDirRaw` registry entry next to its sibling `WMET_WSpeedRaw`, re-added the missing `"WMET_WDirRaw": round(nac_vane_raw, 2)` output dict assignment, and deleted the three orphaned lines. Diff +6/−3 in 2 files; ruff `All checks passed!`; 16/16 self-test PASS (#119 baseline + #125 Glauert γ projection + ±8° clamp + 360° wrap + double monotonicity); SCADA registry restored to 110 entries (104 physics + 6 system).
