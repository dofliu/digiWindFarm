...
- nacelle wind vane transfer function (WVTF) — fixed: IEC 61400-12-2 Annex E swirl-bias model `θ_swirl ≈ Ct / (2·λ)` rad (Burton et al. 2011, Wind Energy Handbook §3.7) for right-handed rotor; Region 2 (Ct≈0.82, λ≈7) → +3.4°, Region 3 (Ct≈0.30, λ≈5) → +1.7°, stopped → 0°; clamp ±8°; reuses `aero_out.ct` and `aero_out.tsr` already computed in `step()` (no extra cost); `WMET_WDirAbs` keeps free-stream semantics (wake-model upstream indexing + yaw-controller backwards compat), new `WMET_WDirRaw` exposes the as-measured vane reading — see #119
