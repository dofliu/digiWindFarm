...
- [x] Nacelle wind vane transfer function (#119, IEC 61400-12-2 Annex E): real cup/wind vane on top of nacelle reads a systematic **swirl bias** from the rotor wake. Burton et al. (2011) Wind Energy Handbook §3.7 gives `θ_swirl ≈ Ct / (2·λ)` rad for a right-handed rotor (industry standard, +bias downwind). Operating-state branching: stopped/parked → 0°; producing/starting with `rotor_speed > 1 RPM` and `λ > 1` applies the swirl bias. Self-test (8/8 + double monotonicity): Region 2 (Ct=0.82, λ=7) → +3.36°; Region 2.5 (Ct=0.65, λ=6) → +3.10°; Region 3 (Ct=0.30, λ=5) → +1.72°; starting (Ct=0.55, λ=6) → +2.63°; stopped → 0°; extreme Ct=0.95/λ=4 → +6.80° (under ±8° clamp); 360°-wrap (358° + 3.36° → 1.36°) verified. Monotone Ct↑ → bias↑ and λ↑ → bias↓ across Region 1.5 → Region 3 sweep. Reuses `aero_out.ct` and `aero_out.tsr` already computed in `step()` for #117 (no extra cost, no new RNG mutation). Backwards compatible: existing `WMET_WDirAbs` keeps free-stream semantics so wake-model upstream indexing and yaw-controller logic stay untouched; new `WMET_WDirRaw` exposes the as-measured vane reading (Bachmann OPC suffix `WMET.Z72PLC__UI_Loc_WMET_Analogue_WDirRaw`). Pairs with #117 to complete the IEC 61400-12-1/2 nacelle-sensor transfer-function chain — `WMET_WSpeedRaw` for the anemometer (axial induction), `WMET_WDirRaw` for the vane (rotor swirl). 104 SCADA tags total (was 103): +1 raw nacelle wind vane tag (`WMET_WDirRaw`).
- [x] Nacelle wind vane transfer function (#119, IEC 61400-12-2 Annex E): real wind vane sits ~1.5 R behind hub on top of nacelle and reads systematic swirl bias from rotor wake. Bias formula `θ_s ≈ Ct/(2·λ)` rad (Burton, Sharpe, Jenkins, Bossanyi 2011, *Wind Energy Handbook* 2nd ed. §3.7) derived from angular momentum / `a' = Ct/(4·λ)`. Right-handed rotor (clockwise from upwind, industry standard) → positive bias. Operating-state branching: stopped/parked or `tsr ≤ 1` → bias = 0; producing/starting with `rotor_speed > 1 RPM` and `tsr > 1` applies the swirl correction. Self-test (11/11 + monotonicity): Region 2 (Ct=0.82, λ=7) → +3.36°; Region 3 (Ct=0.30, λ=5) → +1.72°; stopped → 0°; extreme Ct=0.95 / λ=4 → +6.80° (within ±8° clamp); 360° wrap-around 358° + 3.36° → 1.36°. Reuses `aero_out.ct` and `aero_out.tsr` already computed in `step()` (no extra cost, no new RNG mutation, no additional state). Backwards compatible — `WMET_WDirAbs` keeps free-stream semantics so wake indexing, `yaw_model` controller errors and existing analysis scripts (`examples/data_quality_analysis.py`, `server/opc_adapter.py`, frontend wind rose) keep working unchanged; new `WMET_WDirRaw` exposes the as-measured vane reading (Bachmann OPC suffix `WMET.Z72PLC__UI_Loc_WMET_Analogue_WDirRaw`). 104 SCADA tags total (was 103): +1 raw nacelle vane tag (`WMET_WDirRaw`).
- [x] 104 SCADA tags total (was 103): +1 raw nacelle vane tag (`WMET_WDirRaw`) — see #119
- [x] Nacelle wind vane transfer function (#119, IEC 61400-12-2 Annex E): real wind vane on top of nacelle reads a systematic swirl bias from rotor wake rotation. From BEM tangential induction `a' = Ct / (4·λ)` (Burton et al. 2011 *Wind Energy Handbook* §3.7), the swirl angle at the nacelle position is `θ_s ≈ Ct / (2·λ)` rad. Right-handed rotor (industry standard, clockwise from upwind) → +bias. Operating-state branching: producing or starting with `rotor_speed > 1 RPM` and `λ > 1` applies the swirl bias; otherwise bias = 0. Clamp ±8° (Pedersen et al. 2008 Risø-R-1602 measured 3–8° on real machines). Self-test (10/10 PASS): Region 2 peak Cp (Ct=0.82, λ=7) → +3.36°; Region 2.5 (Ct=0.65, λ=6) → +3.10°; Region 3 (Ct=0.30, λ=5) → +1.72°; stopped → 0°; clamp triggers at Ct=0.95, λ=1.5 → +8.00°; 360°-wrap (358° + 3.36° → 1.36°) correct; Ct↑ → bias↑ and λ↑ → bias↓ monotonicity verified. Reuses `aero_out.ct` and `aero_out.tsr` already computed by `power_curve.get_power_cp(...)` — zero extra cost, no new RNG mutation. Backwards compatible — `WMET_WDirAbs` is kept as the free-stream direction so the frontend wind-rose, wake-source-direction indexing, and `yaw_model` control-error logic are all unchanged; new `WMET_WDirRaw` (REAL32, deg, 0–360, OPC suffix `WMET.Z72PLC__UI_Loc_WMET_Analogue_WDirRaw`) exposes the as-measured vane reading. With #117 NTF this completes the IEC 61400-12-1/2 nacelle sensor transfer-function pair: NTF describes "what speed the sensor sees", WVTF describes "what direction the sensor sees".
