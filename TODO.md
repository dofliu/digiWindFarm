...
- [x] 2026-05-03 regression fix: restore `WMET_WDirRaw` (#119 WVTF) dropped by #125's "duplicate cleanup" merge (the registry only had one entry; the F601 was from the dict literal). Concurrently cleaned the orphaned `yaw_skew_rad` / `yaw_skew_cos` / `induction_a_yawed` lines (F841 × 2) leftover from #127 merge that duplicated the `cos_gamma` block. Diff +6/−3 in `scada_registry.py` and `turbine_physics.py`. ruff `All checks passed!` (was 2 × F841); 16/16 self-test PASS in `/tmp/test_wdir_raw_restore.py` covering #119 baseline (Region 2 → +3.36°, Region 3 → +1.72°, stopped → 0°), #125 Glauert γ projection (γ=15°/30°/45° → 3.24°/2.91°/2.37°, ±15° symmetry, |γ|↑ → bias↓ monotonic, γ=60° → clamp to 45°), Ct↑ → bias↑ + λ↑ → bias↓ + ±8° clamp + 360° wrap-around. SCADA registry restored to 110 entries (104 physics + 6 system).
