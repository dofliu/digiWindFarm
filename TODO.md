...
- [x] Glauert yaw skewed-flow correction on NTF + WVTF (#125, Glauert 1935 / Burton et al. 2011 §3.7+§3.10): closes the IEC 61400-12-1/2 nacelle-sensor chain when the rotor is yawed. NTF uses `a_skew = a · cos²(γ)` (Glauert/Coleman skewed-momentum), giving `V_raw = V_∞ · (1 − 0.55·a·cos²(γ))`; WVTF uses `θ_s_eff = (Ct/(2·λ)) · cos(γ)` (planar projection of the tangential-induction vector onto the nacelle plane). Both reuse the same single `cos(γ)` evaluation and the `yaw_out["yaw_error"]` already computed by `yaw.step(...)`, so any source of yaw error — `yaw_misalignment` faults, ±5° dead-band steady-state, gust transients, wind-direction step changes — flows automatically. γ clamped to ±45° (validity bound of skewed-momentum theory). Acceptance (18/18 self-test PASS): γ=0° → exact #117/#119 baseline (R2 NTF=0.842, R3=0.955, vane R2=+3.36°, R3=+1.72°); γ=15° → NTF reduction × 0.933 (R2 V_raw 8.42→8.52 m/s @ V_∞=10), WVTF bias × 0.966 (3.36°→3.24°); γ=30° → 0.75 / 0.866; γ=45° → 0.50 / 0.707; γ→−γ symmetry exact (cos is even); monotonicity |γ|↑ → NTF factor↑ / WVTF bias↓ verified across γ ∈ {0,10,20,30,40,45}°. No new SCADA tag — observable via `WMET_WSpeedRaw / WMET_WSpeedNac` ratio against `WYAW_YwVn1AlgnAvg5s` and the `(WMET_WDirRaw − WMET_WDirAbs)` channel. Side effect: removes two duplicate `WMET_WDirRaw` keys leftover from the #119 merge (`turbine_physics.py:777` output dict + `scada_registry.py:151` registry list) — `ruff F601` now clean. References: Glauert (1935) "Airplane Propellers" in Durand *Aerodynamic Theory*; Burton et al. (2011) *Wind Energy Handbook* 2nd ed. §3.7 + §3.10; Castillo-Negro et al. (2008) Coleman skewed-wake correction; IEC 61400-12-1 ed.2 Annex D + IEC 61400-12-2 ed.1 Annex E.
