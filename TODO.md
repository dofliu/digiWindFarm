...
- [x] Wake-added turbulence intensity (Crespo-Hernández 1996 / IEC 61400-1 Annex E; per source `TI_added=0.73·a^0.8325·TI_amb^0.0325·(x/D)^-0.32`, sum-of-squares multi-wake; combined in quadrature with pocket TI; downstream AR(1) σ rises 2.1× at 5D in self-test) — see #103
...
- [x] Wake-added turbulence intensity (Crespo-Hernández 1996): `TI_added(x,r) = 0.73·a^0.8325·TI_amb^0.0325·(x/D)^-0.32 · gauss(r)` with axial induction `a = 0.5·(1−√(1−Ct))`; multi-source via sum-of-squares; combined with `WMET_LocalTi` pocket multiplier in quadrature and fed back to per-turbine AR(1) generator; new `WMET_WakeTi` tag — see #103
