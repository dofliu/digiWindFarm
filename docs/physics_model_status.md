...
Last updated: 2026-05-03 (regression fix: restore WMET_WDirRaw tag dropped by #125 cleanup)
...
Newly implemented:
- Yaw-skew Glauert correction for NTF + WVTF (#125, IEC 61400-12-1/2 / Glauert (1935) "Airplane Propellers" in *Aerodynamic Theory* (Durand) §VII / Burton, Sharpe, Jenkins, Bossanyi 2011 *Wind Energy Handbook* 2nd ed. §3.7 / §3.10 / Castillo-Negro et al. 2008 — Coleman skewed wake correction): the original #117 NTF and #119 WVTF blocks both assumed γ=0 (zero yaw misalignment), but real operational data routinely has γ ∈ ±5° (yaw control dead-band) and γ ∈ ±15–25° during `yaw_misalignment` fault, vane miscalibration, or fast wind direction changes. Under non-zero yaw, Glauert combined-momentum theory says the rotor sees only `V·cos(γ)` axially, so the axial induction at the anemometer position reduces to `a_eff = a · cos²(γ)` (Glauert combined-momentum / Coleman skewed-wake correction); identically, the rotor swirl plane (perpendicular to the rotor axis) tilts relative to the nacelle plane, and the wind vane reads only the projection `θ_s_eff = θ_s · cos(γ)`. Both reuse the `yaw_out["yaw_error"]` already in `step()` (combined fault bias + sensor bias + control error), so there is zero extra computation, no new RNG draw, no new state variable, and no new SCADA tag. γ is clamped to ±45° to keep the cos-law inside the small-yaw regime where Glauert's tangent-formula matches measurement; beyond ±45° more elaborate Coleman correction is required and the sensor reading saturates anyway. γ=0° reproduces the prior #117/#119 behaviour bit-for-bit (cos²(0)=cos(0)=1). Self-test (14/14 PASS, closed-form replay of the inserted formulas): Region 2 / γ=0° → factor=0.842 (matches #117 baseline); γ=15° → 0.852 (NTF correction shrinks 6.7%, exactly cos²(15°)=0.933 of original); γ=30° → 0.881 (correction shrinks 25%, cos²(30°)=0.75); γ=45° → 0.921 (correction halved, cos²(45°)=0.5); WVTF γ=0° → +3.36° (matches #119), γ=15° → +3.24° (cos(15°)=0.966), γ=30° → +2.91° (cos(30°)=0.866), γ=45° → +2.38° (cos(45°)=0.707); negative-γ symmetry (γ=−15° gives same factor as γ=+15° because cos² is even); ±45° clamp triggers at γ=60°; γ↑ → factor↑ and bias↓ both strictly monotonic on a 0/10/20/30/40° sweep. Operational impact: under the `yaw_misalignment` fault scenario (which biases yaw_out["yaw_error"] by 15°+) the as-measured `WMET_WSpeedRaw` reading rises towards `WMET_WSpeedNac` (the rotor "looks smaller" axially), and `WMET_WDirRaw - WMET_WDirAbs` shrinks below the swirl baseline — these are exactly the signatures field engineers use to diagnose vane-miscalibration / systematic-yaw-bias failure modes. Backwards compat: the free-stream tags `WMET_WSpeedNac` and `WMET_WDirAbs` are intentionally left as free-stream signals so `wind_field.py` upstream-wake indexing, `yaw_model.py` control-error computation, the frontend wind rose, and OPC adapter consumers are all unchanged. No new SCADA tag — observable via `(WMET_WSpeedRaw / WMET_WSpeedNac − 1) × WYAW_YwVn1AlgnAvg5s` and `(WMET_WDirRaw − WMET_WDirAbs) × WYAW_YwVn1AlgnAvg5s` correlations, which should now correlate physically with yaw error rather than being constants. With #117 NTF + #119 WVTF, this PR closes the IEC 61400-12-1/2 nacelle-sensor transfer-function chain under non-zero yaw error and forms the physical foundation for "vane miscalibration → systematic yaw error → power loss" RAG diagnostic flow (#51).
...
- #125: removed duplicate `WMET_WDirRaw` registration left by the #119 merge in two places — the dict literal in `turbine_physics.py::step()` output (previously inserted at the WMET_WDirAbs cluster *and* at the new WSpeedRaw cluster) and the `_TAGS` list in `scada_registry.py` (lines 151 *and* 202). Same value, same OPC suffix, same range — the second insert shadowed the first, so behaviour is unchanged but ruff flagged F601 (duplicate dict key). Cleanup keeps the WSpeedRaw-paired definition (next to its IEC 61400-12-1/2 sibling) and removes the parallel-with-WDirAbs duplicate. SCADA tag count stays at 104 physics tags / 110 total registry entries.
- 2026-05-03 (post-merge regression fix, follow-up to #125 + #127): the #125 "duplicate cleanup" merge actually removed *both* copies of `WMET_WDirRaw` from `scada_registry.py` (the only registry entry was at line 148 / 151 — there was no duplicate to remove; the F601 flag came from the dict literal in `turbine_physics.py::step()`, not the registry). After the merge, the registry was missing the tag entirely while `turbine_physics.py::step()` still computed `nac_vane_raw = (wind_direction + vane_bias_deg) % 360.0` but never published it (ruff F841: unused variable). Net effect: the IEC 61400-12-2 Annex E WVTF reading from #119 silently disappeared from SCADA output, and downstream consumers expecting `WMET_WDirRaw` got a missing-key fallback. Concurrently, the #127 cup-anemometer-overspeeding merge left orphaned `yaw_skew_rad` / `yaw_skew_cos` / `induction_a_yawed` lines (ruff F841 × 1) duplicating the `cos_gamma` / `cos2_gamma` block introduced by #125 immediately above; the active NTF path uses `cos2_gamma`, so the three orphaned lines were dead code. Today's fix (a) re-adds the canonical `ScadaTag("WMET_WDirRaw", ...)` registry entry next to its sibling `WMET_WSpeedRaw` (single insert, no duplicate), (b) adds the missing `"WMET_WDirRaw": round(nac_vane_raw, 2)` to the output dict, and (c) deletes the three orphaned lines. Diff: +6 / −3 across `scada_registry.py` and `turbine_physics.py`. Verification: ruff `All checks passed!` (was 2 × F841), py_compile ok, 16/16 self-test PASS in `/tmp/test_wdir_raw_restore.py` covering #119 baseline (Region 2 → +3.36°, Region 3 → +1.72°, stopped → 0°), #125 Glauert γ projection (γ=15° → 3.24°, γ=30° → 2.91°, γ=45° → 2.37°, γ=60° clamps to 45°, ±15° symmetry, |γ|↑ → bias↓ monotonic), Ct↑ → bias↑ monotonic, λ↑ → bias↓ monotonic, ±8° clamp, 360° wrap-around. SCADA registry restored to 110 entries (104 physics tags + 6 system, matching post-#119/#125 baseline).
