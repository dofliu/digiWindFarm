...
- nacelle wind vane transfer function (WVTF) implemented (IEC 61400-12-2 Annex E: real wind vane on top of nacelle reads a systematic swirl bias from the rotor wake — the rotor converts linear inflow into linear+rotational outflow by angular-momentum conservation. Burton et al. 2011 *Wind Energy Handbook* §3.7 derives `θ_swirl ≈ Ct / (2·λ)` rad from blade-element-momentum theory; modern utility-scale turbines are right-handed (clockwise from upwind), giving a positive vane bias. Reuses `aero_out.ct` and `aero_out.tsr` already computed for #117 NTF (no extra cost). Self-test (8/8 + double monotonicity): Region 2 (Ct=0.82, λ=7) → +3.36°; Region 2.5 (Ct=0.65, λ=6) → +3.10°; Region 3 (Ct=0.30, λ=5) → +1.72°; starting → +2.63°; stopped → 0°; clamp ±8°; 360°-wrap ✓; Ct↑ → bias↑ and λ↑ → bias↓ verified. Backwards compatible — `WMET_WDirAbs` keeps free-stream semantics so `wind_field.py` upstream-turbine indexing, `yaw_model.py` controller error, frontend wind rose all keep working unchanged; new `WMET_WDirRaw` SCADA tag exposes the as-measured vane reading. Pairs with #117 to complete the IEC 61400-12-1/2 nacelle-sensor transfer-function chain — `WMET_WSpeedRaw` for the anemometer (axial induction reduces apparent speed), `WMET_WDirRaw` for the vane (rotor swirl shifts apparent direction). 104 SCADA tags total — see #119)
- nacelle wind vane transfer function (WVTF) implemented (IEC 61400-12-2 Annex E: real wind vane on top of the nacelle reads systematic swirl bias from rotor wake. Bias formula `θ_s ≈ Ct/(2·λ)` rad following Burton, Sharpe, Jenkins, Bossanyi 2011 *Wind Energy Handbook* §3.7 (angular momentum / `a' = Ct/(4·λ)` from blade element + momentum theory). Right-handed rotor (clockwise from upwind, industry standard) → positive bias. Reuses `aero_out.ct` and `aero_out.tsr` already computed in `step()` (no extra cost, no new RNG mutation). Self-test (11/11 + monotonicity): stopped → 0°; starting (Ct=0.55, λ=6) → +2.63°; Region 2 peak Cp (Ct=0.82, λ=7) → +3.36°; Region 2.5 (Ct=0.65, λ=6) → +3.10°; Region 3 (Ct=0.30, λ=5) → +1.72°; extreme Ct=0.95 / λ=4 → +6.80° within ±8° clamp; 360° wrap-around 358° + 3.36° → 1.36°; Ct↑ → bias↑ and λ↑ → bias↓ both monotonic. Backwards compatible — `WMET_WDirAbs` keeps free-stream semantics so wake indexing, `yaw_model` controller errors and existing wind-rose / data-quality scripts still see the same value; new `WMET_WDirRaw` SCADA tag exposes the as-measured vane reading (Bachmann OPC suffix `WMET.Z72PLC__UI_Loc_WMET_Analogue_WDirRaw`) for IEC 61400-12-2 nacelle-mounted vane studies and future vane-miscalibration fault scenarios. 104 SCADA tags total — see #119)
